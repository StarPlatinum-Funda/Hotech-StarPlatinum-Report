...
| RNF11  | El sistema debe mostrar el estado de habitaciones en tiempo real sin recargar la página      | Al menos el 95% de las actualizaciones de estado se reflejan en pantalla en menos de 3 segundos                          | EP05 - HU03             | Eficiencia / Usabilidad |
| RNF12  | La plataforma debe garantizar que las tareas asignadas no puedan ser vistas por empleados no autorizados | Los empleados sólo pueden ver tareas que les fueron asignadas mediante roles de acceso configurados                      | EP03 - HU02             | Seguridad               |
| RNF13  | El sistema debe mantener el historial de limpieza de cada habitación por al menos 30 días    | Se puede consultar un registro de tareas de limpieza completadas para cualquier habitación dentro del último mes         | EP05 - HU04             | Auditabilidad / Mantenibilidad |
| RNF14  | Las notificaciones de tareas deben ser persistentes y con confirmación de lectura            | Cada notificación enviada se marca como “leída” una vez abierta, y se almacena el tiempo de lectura                      | EP04 - HU03             | Fiabilidad / Usabilidad |
| RNF15  | El sistema debe tener logs de actividad por usuario                                           | Se registra cada acción clave (login, asignación, modificación) con ID de usuario y timestamp                           | EP06, EP07              | Trazabilidad / Seguridad |
| RNF16  | El sistema debe permitir uso offline parcial para empleados de limpieza                      | En modo offline, los empleados pueden consultar tareas y registrar estado, y la data se sincroniza al reconectar         | EP03 - HU04             | Disponibilidad / Usabilidad |
| RNF17  | La plataforma debe contar con diseño inclusivo y accesible según WCAG nivel AA               | Cumple con las pautas WCAG 2.1 AA (etiquetas claras, navegación con teclado, contraste de color adecuado)                | EP01 - HU01             | Accesibilidad           |
| RNF18  | El sistema debe poder integrarse en el futuro con soluciones de control de acceso (cerraduras inteligentes) | El backend expone endpoints REST para conectividad futura con dispositivos IoT                                           | EP07                    | Escalabilidad / Interoperabilidad |
| RNF19  | El sistema debe soportar múltiples negocios por cuenta (modelo multi-tenant básico)          | Un usuario administrador puede gestionar más de un establecimiento desde su cuenta si así lo requiere                   | EP01 - HU01             | Escalabilidad / Seguridad |
| RNF20  | El sistema debe tener tiempos de respuesta consistentes en conexiones inestables             | En conexiones con ancho de banda limitado, las funcionalidades clave siguen operativas en el 90% de los intentos        | Todas las historias     | Robustez               |
